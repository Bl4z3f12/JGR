            ...
            <div class="row">
                <div class="col-12">
                    ...
                    <div class="row g-4 mb-4">
                        <?php foreach ($stage_options as $stage_name): 
                            $incoming = $incoming_data[$stage_name] ?? 0;
                            $outgoing = $outgoing_data[$stage_name] ?? 0;
                            $current = max(0, $incoming - $outgoing);
                            ...
                            $incoming_percent = ($total_count ?? 0) > 0 ? round(($incoming / ($total_count ?? 1)) * 100) : 0;
                            $outgoing_percent = ($total_count ?? 0) > 0 ? round(($outgoing / ($total_count ?? 1)) * 100) : 0;
                            $current_percent = ($total_count ?? 0) > 0 ? round(($current / ($total_count ?? 1)) * 100) : 0;
                        ?>
                                ...
                                <div class="card border-0 shadow-sm h-100 <?= $bgClass ?> text-white">
                                    <div class="card-body">
                                        ...
                                        <div class="stage-stats">
                                            <div class="stat-item">
                                                <div class="stat-value"><?= number_format($incoming) ?></div>
                                                <div class="stat-label">In</div>
                                            </div>
                                            <div class="stat-item">
                                                <div class="stat-value"><?= number_format($outgoing) ?></div>
                                                <div class="stat-label">Out</div>
                                            </div>
                                            <div class="stat-item">
                                                <div class="stat-value"><?= number_format($current) ?></div>
                                                <div class="stat-label">Current</div>
                                            </div>
                                        </div>
                                        
                                        <div class="progress-container mt-3">
                                            <div class="progress-bar-incoming" style="width: <?= min(100, ($incoming/1000)*100) ?>%;"></div>
                                            <div class="progress-bar-outgoing" style="width: <?= min(100, ($outgoing/1000)*100) ?>%;"></div>
                                        </div>
                                        
                                        <div class="progress-labels text-white-50">
                                            <small>In: <?= $incoming_percent ?>%</small>
                                            <small>Out: <?= $outgoing_percent ?>%</small>
                                            <small>Current: <?= $current_percent ?>%</small>
                                        </div>
                                        
                                        <div class="text-center mt-3">
                                            <span class="badge <?= getBadgeColor($current) ?> rounded-pill px-3 py-2">
                                                <?php if($current > 900): ?>
                                                    Excellent
